...
export const useGameState = () => {
  ...
  const createDefaultGameStateWrapped = useCallback((options?: Partial<EraInitOptions>): GameState => ({
    money: options?.startingMoney || 2000,
    reputation: 10,
    currentDay: 2,
    currentYear: options?.currentYear || 1960, // Start in 1960s era
  ...
  const initializeGameState = useCallback((options?: Partial<EraInitOptions>): GameState => {
    const newGameState = createDefaultGameStateWrapped(options);
  ...
  return {
    gameState: gameState || createDefaultGameStateWrapped(),
    setGameState: (state: GameState | ((prev: GameState) => GameState)) => {
      if (typeof state === 'function') {
        setGameState(prev => state(prev || createDefaultGameStateWrapped()));
      } else {
        setGameState(state);
      }
    },
